...
//|              Omni-B3 EA v2.61 — Smart Close para B3/NETTING       |
    ...
    bool CheckSmartClose(SGridState &state) {
        ...
        if(state.positive_profit_sum >= required) {
            ...
            m_logger.Info("SmartClose",
                ...
                StringFormat("[ALVO] Smart Close! Lucros=R$%.2f | Necessário=R$%.2f | Pior=R$%.2f",
    ...
    bool CheckBreakEven(SGridState &state) {
        ...
        if(state.total_profit >= acceptable_money) {
            ...
            m_logger.Info("SmartClose",
                ...
                StringFormat("[ALVO] BreakEven! P&L=R$%.2f | Aceitável=R$%.2f",
        ...
        if(m_lot_sum_total > 0.0 && state.total_volume >= m_lot_sum_total) {
            ...
            if(state.total_profit >= m_min_profit) {
                ...
                m_logger.Info("SmartClose",
                    ...
                    StringFormat("[ALVO] Lote Total! Vol=%.0f (máx=%.0f)", state.total_volume, m_lot_sum_total));
        ...
        if(m_order_count_total > 0 && state.total_levels >= m_order_count_total) {
            ...
            if(state.total_profit >= m_min_profit) {
                ...
                m_logger.Info("SmartClose",
                    ...
                    StringFormat("[ALVO] Qtde Total! Ordens=%d (máx=%d)", state.total_levels, m_order_count_total));
    ...
    bool CheckAcceptLoss(SGridState &state) {
        ...
        if(state.total_profit >= m_accept_loss_value) {
            ...
            m_logger.Info("SmartClose",
                ...
                StringFormat("[ALVO] Aceitar Perda! P&L=R$%.2f | Aceitável=R$%.2f | DD=%.1f%%",
...
//| Força o fechamento incondicional de todas as posições        |
...
bool CloseAllPositions() {
    ...
    m_logger.Info("SmartClose", "[FORÇADO] Executando fechamento incondicional de todas as posições.");
...
//| Configura TakeProfit                                          |
            ...
            if(!m_trail_active) {
                ...
                if(profit_pts >= m_trail_trigger) {
                    ...
                    m_logger.Info("SmartClose",
                        ...
                        StringFormat("ðŸ”¥ Trailing Virtual Grade Ativado na COMPRA! Lucro: %.0f pts. Preço: %.2f | SL: %.2f | TP: %.2f", 
            ...
            if(m_trail_active) {
                ...
                if(current_bid <= m_virtual_sl) {
                    ...
                    m_logger.Info("SmartClose",
                        ...
                        StringFormat("[ALERTA] Trailing Virtual COMPRA atingido pelo Stop Gain! Preço: %.2f <= SL Virtual: %.2f. Fechando a grade inteira.", 
                ...
                if(current_bid >= m_virtual_tp) {
                    ...
                    m_logger.Info("SmartClose",
                        ...
                        StringFormat("[ALVO] Trailing Virtual COMPRA atingido pelo Gain Móvel! Preço: %.2f >= TP Virtual: %.2f. Fechando a grade inteira.", 
            ...
            if(!m_trail_active) {
                ...
                if(profit_pts >= m_trail_trigger) {
                    ...
                    m_logger.Info("SmartClose",
                        ...
                        StringFormat("ðŸ”¥ Trailing Virtual Grade Ativado na VENDA! Lucro: %.0f pts. Preço: %.2f | SL: %.2f | TP: %.2f", 
            ...
            if(m_trail_active) {
                ...
                if(current_ask >= m_virtual_sl) {
                    ...
                    m_logger.Info("SmartClose",
                        ...
                        StringFormat("[ALERTA] Trailing Virtual VENDA atingido pelo Stop Gain! Preço: %.2f >= SL Virtual: %.2f. Fechando a grade inteira.", 
                ...
                if(current_ask <= m_virtual_tp) {
                    ...
                    m_logger.Info("SmartClose",
                        ...
                        StringFormat("[ALVO] Trailing Virtual VENDA atingido pelo Gain Móvel! Preço: %.2f <= TP Virtual: %.2f. Fechando a grade inteira.", 
            ...
            if(effective_tp > 0.0 || m_tp_acceptable < 0.0) {
                ...
                if(state.total_profit >= effective_tp) {
                    ...
                    m_logger.Info("SmartClose",
                        ...
                        StringFormat("ðŸ›¡ï¸? Salvaguarda TP Total atingida! P&L=R$%.2f | TP=R$%.2f. Fechando toda a grade por segurança.", state.total_profit, effective_tp));
    ...
    //--- Getters para trailing stop virtual da grade (OmniB3 v2.60)
