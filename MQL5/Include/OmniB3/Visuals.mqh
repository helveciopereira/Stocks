//+------------------------------------------------------------------+
//|                                                      Visuals.mqh |
//|                     Omni-B3 EA v2.45 — Módulo Visual Avançado     |
...
bool CVisuals::Init(CLogger *logger, int magic_number, string symbol) {
    ...
    ChartSetInteger(m_chart_id, CHART_SHOW_OBJECT_DESCRIPTIONS, true);

    // Desenha o histórico inicial acumulado de todos os dias operados
    DrawTradeHistory();

    if(m_logger != NULL) m_logger.Info("Visuals", "Módulo de desenho gráfico premium inicializado para v2.45.");
    return true;
}
...
void CVisuals::DrawTradeHistory() {
    ...
    // Chave: ID da Posição, Valor: Índice do Deal ou Preço/Tempo correspondente
    ...
    for(int i = 0; i < total_deals; i++) {
        ...
        if(entry_type == DEAL_ENTRY_OUT || entry_type == DEAL_ENTRY_OUT_BY) {
            ...
            if(entry_idx != -1) {
                ...
                // 4. Texto Flutuante com o Valor Monetário Obtido
                if(ObjectFind(m_chart_id, text_lbl_name) < 0) {
                    // Calcula um offset vertical leve para o texto não sobrepor a seta (ex: 20 pontos de WIN)
                    double offset = 0.0;
