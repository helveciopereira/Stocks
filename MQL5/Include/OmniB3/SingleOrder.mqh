...
public:
    ...
    // Retorna a direção da posição física ativa (1 = COMPRA, -1 = VENDA, 0 = SEM POSIÇÃƒO)
...
//| (1 = COMPRA, -1 = VENDA, 0 = SEM POSIÇÃƒO)                        |
//+------------------------------------------------------------------+
    ...
    if(pos_type == POSITION_TYPE_BUY) {
        ...
        if(!m_trail_active) {
            if(profit_pts >= m_trail_trigger) {
                ...
                if(m_logger != NULL) {
                    m_logger.Info("SingleOrder", StringFormat("ðŸ”¥ Trailing Físico Ativado na COMPRA! Lucro: %.0f pts. Preço: %.2f", profit_pts, current_bid));
                }
            }
        }
    ...
    // VENDA
        ...
        if(!m_trail_active) {
            if(profit_pts >= m_trail_trigger) {
                ...
                if(m_logger != NULL) {
                    m_logger.Info("SingleOrder", StringFormat("ðŸ”¥ Trailing Físico Ativado na VENDA! Lucro: %.0f pts. Preço: %.2f", profit_pts, current_ask));
                }
            }
        }
