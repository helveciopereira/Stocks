//+------------------------------------------------------------------+
//|                                                   SingleOrder.mqh |
//|                     Omni-B3 EA v2.47 â€” Modo de Ordem Ãšnica        |
...
//+------------------------------------------------------------------+
//| CLASSE CSingleOrder                                              |
//| Gerencia ordens simples independentes sem formaÃ§Ã£o de grade      |
//+------------------------------------------------------------------+
class CSingleOrder {
private:
    CTrade              m_trade;            // Classe padrÃ£o de trade do MT5
    ulong               m_magic;            // NÃºmero MÃ¡gico
    ENUM_SINGLE_ORDER_MODE m_mode;          // Modo ativo (Habilitado/Desabilitado)
    
    // ConfiguraÃ§Ãµes do Trade
    double              m_sl_points;        // StopLoss em pontos (0 = sem)
    double              m_tp_points;        // TakeProfit em pontos (0 = sem)
    double              m_be_activation;    // AtivaÃ§Ã£o do BreakEven em pontos
    ...
    int                 m_mart_max_steps;   // Limite de multiplicaÃ§Ãµes consecutivas
    ...
    // LÃ³gicas de Espera (Cooldown)
    int                 m_wait_after_loss;  // Segundos de espera apÃ³s uma perda
    int                 m_wait_after_win;   // Segundos de espera apÃ³s um ganho
    datetime            m_last_close_time;  // Hora do fechamento do Ãºltimo trade
    double              m_last_trade_profit;// Lucro do Ãºltimo trade fechado

    // Auxiliares internos
    bool                m_be_applied;       // Sinalizador de BreakEven aplicado
    bool                m_close_on_opposite;// Fechar posiÃ§Ã£o se houver sinal contrÃ¡rio?
    CLogger            *m_logger;           // Ponteiro para o Logger centralizado

    // Sistema de Trailing (Gain / Stop Gain MÃ³veis)
    bool                m_use_trailing;     // Habilitar trailing mÃ³vel
    ...
    double              m_max_bid_seen;     // Maior preÃ§o Bid visto no trade de COMPRA
    double              m_min_ask_seen;     // Menor preÃ§o Ask visto no trade de VENDA

    // Rastreia o histÃ³rico para saber o resultado do Ãºltimo trade do Magic Number
...
public:
    ...
    // Retorna a direÃ§Ã£o da posiÃ§Ã£o fÃ­sica ativa (1 = COMPRA, -1 = VENDA, 0 = SEM POSIÃ‡ÃƒO)
...
CSingleOrder::CSingleOrder() {
    ...
    // InicializaÃ§Ã£o do Trailing mÃ³vel
...
//+------------------------------------------------------------------+
//| Retorna a direÃ§Ã£o da posiÃ§Ã£o fÃ­sica ativa                         |
//| (1 = COMPRA, -1 = VENDA, 0 = SEM POSIÃ‡ÃƒO)                        |
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
//| InicializaÃ§Ã£o das ConfiguraÃ§Ãµes                                  |
//+------------------------------------------------------------------+
void CSingleOrder::Init(CLogger *logger,
    ...
    // Atualiza estatÃ­sticas iniciais com base no histÃ³rico da conta
    UpdateHistoryStats();
}

//+------------------------------------------------------------------+
//| Atualiza estatÃ­sticas baseadas no histÃ³rico da conta             |
//+------------------------------------------------------------------+
void CSingleOrder::UpdateHistoryStats() {
    if(m_magic == 0) return;

    // Solicita o histÃ³rico de negociaÃ§Ã£o da conta
    ...
    // Varre de trÃ¡s para frente para pegar os Ãºltimos resultados fechados
    for(int i = total_deals - 1; i >= 0; i--) {
        ...
        // Apenas negÃ³cios de saÃ­da (que fecharam posiÃ§Ã£o) nos interessam
        ...
        // Define a data do Ãºltimo fechamento se for o mais recente
        ...
        // Se encontramos uma transiÃ§Ã£o, finalizamos a busca da sequÃªncia atual
...
//+------------------------------------------------------------------+
//| Verifica se passou o Cooldown de espera apÃ³s ganho/perda          |
//+------------------------------------------------------------------+
bool CSingleOrder::CanOpenNewOrder(datetime current_time) {
    if(m_mode == SINGLE_DISABLED) return false;
    if(m_last_close_time == 0) return true; // Sem histÃ³rico, pode operar

    int elapsed = (int)(current_time - m_last_close_time);

    // Se o Ãºltimo trade foi perdedor e temos cooldown de perda
    ...
    // Se o Ãºltimo trade foi vencedor e temos cooldown de ganho
...
double CSingleOrder::CalculateLot(double initial_lot, double lot_min, double lot_max) {
    // ForÃ§a a atualizaÃ§Ã£o do histÃ³rico antes de calcular o lote
    UpdateHistoryStats();

    double calculated_lot = initial_lot * m_current_multiplier;
    
    // Limita os valores aos limites mÃ­nimos e mÃ¡ximos da corretora/EA
...
bool CSingleOrder::OpenOrder(string symbol, int direction, double volume, string comment) {
    ...
    // Garante que nÃ£o hÃ¡ nenhuma posiÃ§Ã£o aberta com o Magic Number
    ...
    // Define preÃ§os e nÃ­veis de SL/TP com base na direÃ§Ã£o
    ...
    if(ret_code == TRADE_RETCODE_DONE || ret_code == TRADE_RETCODE_PLACED) {
        if(m_logger != NULL) {
            m_logger.Info("SingleOrder", StringFormat("PosiÃ§Ã£o aberta com sucesso. Lote: %.0f, PreÃ§o: %.2f, SL: %.2f, TP: %.2f", 
                                      volume, price, sl, tp));
        }
        return true;
    } else {
        if(m_logger != NULL) {
            m_logger.Error("SingleOrder", StringFormat("Falha ao abrir ordem. CÃ³digo de retorno: %d", ret_code));
        }
        return false;
    }
}
...
//+------------------------------------------------------------------+
//| Gerenciamento de BreakEven para a posiÃ§Ã£o individual             |
//+------------------------------------------------------------------+
void CSingleOrder::ManageBreakEven(string symbol, double current_bid, double current_ask, double tick_size) {
    ...
    if(pos_type == POSITION_TYPE_BUY) {
        // DistÃ¢ncia que o preÃ§o subiu acima da entrada
        double ppts = (current_bid - open_price);
        if(ppts >= m_be_activation) {
            double new_sl = open_price + m_be_margin;
            // SÃ³ ajusta se o SL for menor do que o novo SL de seguranÃ§a
        ...
        // DistÃ¢ncia que o preÃ§o caiu abaixo da entrada
        double ppts = (open_price - current_ask);
        if(ppts >= m_be_activation) {
            double new_sl = open_price - m_be_margin;
            // SÃ³ ajusta se o SL for maior ou nÃ£o definido
...
//+------------------------------------------------------------------+
//| Fecha a PosiÃ§Ã£o se houver Sinal ContrÃ¡rio                         |
//+------------------------------------------------------------------+
bool CSingleOrder::CheckOppositeSignalClose(string symbol, int opposite_signal) {
    ...
    // Se temos posiÃ§Ã£o de compra e o sinal contrÃ¡rio Ã© de VENDA (-1)
    if(pos_type == POSITION_TYPE_BUY && opposite_signal == -1) {
        should_close = true;
    }
    // Se temos posiÃ§Ã£o de venda e o sinal contrÃ¡rio Ã© de COMPRA (1)
    ...
    if(should_close) {
        ...
        // ForÃ§a a atualizaÃ§Ã£o do histÃ³rico
        UpdateHistoryStats();
        return true;
    }
...
//+------------------------------------------------------------------+
//| Gerenciamento do Trailing MÃ³vel FÃ­sico para Ordem Ãšnica          |
//+------------------------------------------------------------------+
void CSingleOrder::ManageTrailing(string symbol, double current_bid, double current_ask, double tick_size) {
    ...
    // Tenta selecionar a posiÃ§Ã£o correspondente ao Magic Number
    if(!PositionSelect(symbol)) return;
    long pos_magic = PositionGetInteger(POSITION_MAGIC);
    if(pos_magic != m_magic) return;
    ...
    if(pos_type == POSITION_TYPE_BUY) {
        ...
        // Se o trailing ainda nÃ£o foi ativado, verifica o gatilho (trigger)
        if(!m_trail_active) {
            if(profit_pts >= m_trail_trigger) {
                m_trail_active = true;
                m_max_bid_seen = current_bid;
                if(m_logger != NULL) {
                    m_logger.Info("SingleOrder", StringFormat("ðŸ”¥ Trailing FÃ­sico Ativado na COMPRA! Lucro: %.0f pts. PreÃ§o: %.2f", profit_pts, current_bid));
                }
            }
        }
        ...
        if(m_trail_active) {
            ...
            // O Stop Gain sÃ³ pode subir. Verificamos se o novo SL ideal estÃ¡ acima do SL atual
            // e se a diferenÃ§a Ã© de pelo menos m_trail_step
            ...
            // O Take Profit mÃ³vel tambÃ©m caminha na direÃ§Ã£o da alta
            bool modify_tp = false;
            if(current_tp == 0.0 || target_tp >= current_tp + m_trail_step) {
                modify_tp = true;
            }

            // Modifica se houver alteraÃ§Ã£o pendente
            if(modify_sl || modify_tp) {
                ...
                if(m_trade.PositionModify(symbol, new_sl, new_tp)) {
                    if(m_logger != NULL) {
                        m_logger.Info("SingleOrder", StringFormat("âš¡ Trailing COMPRA atualizado. SL: %.2f (antigo: %.2f) | TP: %.2f (antigo: %.2f)", new_sl, current_sl, new_tp, current_tp));
                    }
                } else {
                    if(m_logger != NULL) {
                        m_logger.Error("SingleOrder", StringFormat("âŒ Falha ao modificar Trailing COMPRA. CÃ³digo: %d", m_trade.ResultRetcode()));
                    }
                }
            }
        }
    ...
    // VENDA
        ...
        if(!m_trail_active) {
            if(profit_pts >= m_trail_trigger) {
                ...
                if(m_logger != NULL) {
                    m_logger.Info("SingleOrder", StringFormat("ðŸ”¥ Trailing FÃ­sico Ativado na VENDA! Lucro: %.0f pts. PreÃ§o: %.2f", profit_pts, current_ask));
                }
            }
        }
        ...
        if(m_trail_active) {
            ...
            // O Stop Gain na venda sÃ³ pode descer. Verificamos se o novo SL ideal estÃ¡ abaixo do SL atual
            // e se a diferenÃ§a Ã© de pelo menos m_trail_step
            bool modify_sl = false;
            if(current_sl == 0.0 || target_sl <= current_sl - m_trail_step) {
            ...
            // O Take Profit mÃ³vel na venda tambÃ©m caminha na direÃ§Ã£o da baixa
            bool modify_tp = false;
            if(current_tp == 0.0 || target_tp <= current_tp - m_trail_step) {
                modify_tp = true;
            }
            ...
            if(modify_sl || modify_tp) {
                ...
                if(m_trade.PositionModify(symbol, new_sl, new_tp)) {
                    if(m_logger != NULL) {
                        m_logger.Info("SingleOrder", StringFormat("âš¡ Trailing VENDA atualizado. SL: %.2f (antigo: %.2f) | TP: %.2f (antigo: %.2f)", new_sl, current_sl, new_tp, current_tp));
                    }
                } else {
                    if(m_logger != NULL) {
                        m_logger.Error("SingleOrder", StringFormat("âŒ Falha ao modificar Trailing VENDA. CÃ³digo: %d", m_trade.ResultRetcode()));
                    }
                }
            }
        }
