    ...
    void CheckDayReset() {
        ...
        if(now.day != m_last_day) {
            ...
            m_logger.Info("RiskManager",
                StringFormat("ðŸ”„ Novo dia — Saldo=R$%.2f | Equity=R$%.2f",
    ...
    bool IsSafeToTrade(int current_levels) {
        ...
        if(balance > 0 && (equity / balance * 100.0) < m_equity_stop_percent) {
            m_logger.Critical("RiskManager",
                StringFormat("ðŸš¨ EQUITY STOP! Capital Líquido (R$%.2f) caiu abaixo do limite de %.1f%% do Saldo (R$%.2f) — Atual: %.1f%%. Ativando Kill-Switch!",
        ...
        if(m_initial_balance > 0) {
            ...
            if(m_limit_profit_daily > 0.0 && (equity - m_initial_balance) >= m_limit_profit_daily) {
                m_logger.Info("RiskManager",
                    StringFormat("âœ… Meta diária de lucro atingida! Lucro acumulado hoje: R$%.2f (Meta: R$%.2f). Operações encerradas hoje.",
        ...
        if(used_margin > 0) {
            ...
            if(margin_level < m_min_margin_percent) {
                m_logger.Warning("RiskManager",
                    StringFormat("ðŸš¨ Margem Livre Insuficiente na Corretora! Margem livre calculada em %.1f%% do capital garantido, inferior ao limite de segurança configurado de %.1f%%. Novas entradas bloqueadas para evitar Stop Out compulsório pela corretora.", 
