//+------------------------------------------------------------------+
//|                                                 RiskManager.mqh  |
//|                Omni-B3 EA v2.49 — Gestão de Risco (B3/BRL)        |
...
public:
    ...
    bool IsSafeToTrade(int current_levels) {
        ...
        if(m_initial_balance > 0) {
            ...
            if(m_max_daily_dd_percent > 0 && daily_dd > m_max_daily_dd_percent) {
                m_logger.Warning("RiskManager",
                    StringFormat("âš ï¸? DD Diário Atingido: %.2f%% de drawdown (Limite Máximo: %.2f%%). Bloqueando operações diárias para preservar capital inicial de R$%.2f.",
            ...
            if(m_limit_loss_daily > 0.0 && daily_loss >= m_limit_loss_daily) {
                m_logger.Warning("RiskManager",
                    StringFormat("âš ï¸? Perda diária limite atingida: R$%.2f de perda flutuante/realizada (Limite Máximo: R$%.2f). Operações bloqueadas hoje.",
            ...
            if(m_limit_profit_daily > 0.0 && (equity - m_initial_balance) >= m_limit_profit_daily) {
                m_logger.Info("RiskManager",
                    StringFormat("[OK] Meta diária de lucro atingida! Lucro acumulado hoje: R$%.2f (Meta: R$%.2f). Operações encerradas hoje.",
        ...
        if(m_limit_orders_daily > 0 && m_daily_order_count >= m_limit_orders_daily) {
            m_logger.Warning("RiskManager",
                StringFormat("âš ï¸? Limite diário de ordens atingido: %d ordens executadas hoje (Limite Máximo: %d). Bloqueando novas entradas.",
        ...
        if(m_limit_wins_daily > 0 && m_daily_wins >= m_limit_wins_daily) {
            m_logger.Warning("RiskManager",
                StringFormat("âš ï¸? Limite diário de vitórias (trades vencedores) atingido: %d vitórias (Limite Máximo: %d). Bloqueando novas entradas.",
        ...
        if(m_limit_losses_daily > 0 && m_daily_losses >= m_limit_losses_daily) {
            m_logger.Warning("RiskManager",
                StringFormat("âš ï¸? Limite diário de perdas (trades perdedores) atingido: %d perdas (Limite Máximo: %d). Bloqueando novas entradas.",
