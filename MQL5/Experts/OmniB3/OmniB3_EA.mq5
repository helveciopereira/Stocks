...
input string           InpSepStopMM = "---- StopLoss do Robô ----";  // â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
input string           InpSepATR = "---- ATR (Grade Dinâmica) ----";  // â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
input string           InpSepNext = "---- Próximo Lote ----";  // â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
input string           InpSepMA = "---- Médias Móveis ----";  // â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
input string           InpSepDaily = "---- Diário ----";  // â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
input string           InpSeparatorVisuals = "â•â•â•â•â•â•â•â• CONFIGURAÇÕES VISUAIS v2.45 â•â•â•â•â•â•â•â•"; // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
input string           InpSeparator9 = "â•â•â•â•â•â•â•â• FASE 2: ORDEM ÚNICA (SINGLE) â•â•â•â•â•â•â•â•"; // â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
...
//+------------------------------------------------------------------+
int OnInit() {
    // â•â•â• 1. Logger (primeiro — todos dependem dele) â•â•â•
    ...
    // â•â•â• 4. Persistência de Estado â•â•â•
    ...
    // â•â•â• 11. FASE 2: Single Order Módulo â•â•â•
    ...
    // â•â•â• 12. FASE 2: News Filter Módulo â•â•â•
    ...
    // â•â•â• 14. OMNI-B3 v2.45: Módulo Visual e Painel de Operações Recentes â•â•â•
    ...
    Logger.Info("EA", "âœ… Inicialização completa! v" + OMNIB3_VERSION);
...
void OnTick() {
    ...
    if(has_news_block) {
        if(news_act == NEWS_ACTION_CLOSE_ALL) {
            Logger.Warning("EA", "ðŸš¨ Notícia ativa com ação FECHAR TUDO. Encerrando operações.");
        ...
        if(state.oldest_level_time > 0) {
            ...
            if(oldest_dt.year < current_dt.year || 
               ...
               (oldest_dt.year == current_dt.year && oldest_dt.mon == current_dt.mon && oldest_dt.day < current_dt.day)) {
                ...
                Logger.Warning("EA", StringFormat("ðŸ›¡ï¸ Salvaguarda Day Trade acionada! Detectada posição antiga de %04d.%02d.%02d. Liquidando toda a grade a mercado.", 
    ...
    // FASE 2: BLOQUEIO PARCIAL DE NOTÃCIA (NÃƒO ABRE NOVA SÉRIE, MAS PERMITE MANTENÇÃƒO DE GRID)
    ...
    // â•â•â• OMNI-B3 v2.45: Atualização em Tempo Real de Linhas Horizontais de Alvos e Histórico â•â•â•
    if(g_initialized) {
...
void OnTimer() {
    ...
    // FASE 2: ATUALIZAÇÃƒO DO DASHBOARD GRÃFICO
    ...
    // FASE 2: ATUALIZAÇÃƒO DO MONITOR DE OPERAÇÕES RECENTES (v2.45)
...
void OnChartEvent(const int id, const long &lparam,
    ...
    if(InpUseDashboard && Dash != NULL) {
        ...
        if(action != "") {
            if(action == "Panic") {
                Logger.Critical("EA", "ðŸš¨ BOTÃƒO PÂNICO PREVENTIVO ACIONADO VIA DASHBOARD!");
                Risk.ActivateKillSwitch();
                PosManager.ClearAllLevels();
    ...
    if(!InpUseDashboard && id == CHARTEVENT_OBJECT_CLICK) {
        if(sparam == "btn_panic") {
            Logger.Critical("EA", "ðŸ”´ PÂNICO clássico pressionado!");
            Risk.ActivateKillSwitch();
        ...
        if(sparam == "btn_reset") {
            Logger.Info("EA", "ðŸŸ¢ RESET clássico pressionado");
            Risk.ResetKillSwitch();
            if(Recovery != NULL) Recovery.Reset();
        }
    }
