...
//+------------------------------------------------------------------+
//| CLASSE CNewsFilter                                               |
//| Gerencia o calendÃ¡rio econÃ´mico integrado do MT5 para a B3       |
//+------------------------------------------------------------------+
class CNewsFilter {
private:
    bool                 m_enabled;          // Filtro ativado?
    ENUM_NEWS_IMPORTANCE m_min_importance;   // ImportÃ¢ncia mÃ­nima para filtrar
    ENUM_NEWS_ACTION     m_action;           // AÃ§Ã£o tomada durante o bloqueio
    int                  m_minutes_before;   // Minutos para bloquear antes do evento
    int                  m_minutes_after;    // Minutos para bloquear apÃ³s o evento
    string               m_currency_filter;  // Moeda filtrada ("BRL", "USD", "ALL")
    
    // Estado de notÃ­cias carregado
    MqlCalendarValue     m_values[];         // Valores dos eventos carregados
    int                  m_total_events;     // Quantidade total de eventos carregados
    datetime             m_last_update_date; // Data da Ãºltima atualizaÃ§Ã£o de dados

    // Estrutura para guardar a prÃ³xima notÃ­cia mais prÃ³xima
...
public:
    ...
    // Carrega/atualiza os dados de calendÃ¡rio do dia
    bool                 UpdateCalendarData();

    // Verifica se o robÃ´ estÃ¡ em perÃ­odo de bloqueio por notÃ­cia
    // Retorna true se houver bloqueio ativo e preenche a aÃ§Ã£o correspondente
    bool                 CheckNewsBlock(datetime current_time, int &out_action);

    // Retorna a struct de estado da prÃ³xima notÃ­cia mais prÃ³xima
...
void CNewsFilter::Init(CLogger *logger,
    ...
    // Deixa em maiÃºsculo para comparaÃ§Ã£o segura
...
//+------------------------------------------------------------------+
//| Carrega as NotÃ­cias do Dia do CalendÃ¡rio Nativo do MT5            |
//+------------------------------------------------------------------+
bool CNewsFilter::UpdateCalendarData() {
    if(!m_enabled) return false;

    datetime today = TimeCurrent();
    // Zera horas, minutos e segundos para pegar do inÃ­cio do dia
    MqlDateTime dt;
    ...
    // Se jÃ¡ atualizamos hoje e temos dados, nÃ£o precisa recarregar toda hora
    if(m_last_update_date == start_of_day && m_total_events > 0) {
        return true;
    }

    // PerÃ­odo de busca: de hoje atÃ© o final do dia
    datetime end_of_day = start_of_day + 86400;

    // Busca o histÃ³rico de notÃ­cias nativo do terminal
...
//+------------------------------------------------------------------+
//| Verifica se um evento do calendÃ¡rio Ã© relevante para o EA        |
//+------------------------------------------------------------------+
bool CNewsFilter::IsEventRelevant(const MqlCalendarValue &value, const MqlCalendarEvent &event, const MqlCalendarCountry &country) {
    ...
    if(m_currency_filter != "ALL") {
        if(currency != m_currency_filter) {
            // Se filtramos WIN/WDO, USD e BRL sÃ£o as moedas relevantes globais
    ...
    // 2. Filtragem por nÃ­vel de importÃ¢ncia (impacto da notÃ­cia)
...
bool CNewsFilter::CheckNewsBlock(datetime current_time, int &out_action) {
    ...
    // Varre todas as notÃ­cias do dia carregadas
    for(int i = 0; i < m_total_events; i++) {
        MqlCalendarEvent event;
        MqlCalendarCountry country;
        
        // Pega detalhes do evento e do paÃ­s correspondente
        if(!CalendarEventById(m_values[i].event_id, event)) continue;
        if(!CalendarCountryById(event.country_id, country)) continue;

        // Verifica se Ã© relevante
        if(!IsEventRelevant(m_values[i], event, country)) continue;

        datetime event_time = m_values[i].time;
        
        // Se a notÃ­cia ainda nÃ£o tem horÃ¡rio definido (ex: dia inteiro)
        if(event_time == 0) continue;

        // Calcula a diferenÃ§a em segundos para o evento
        ...
        // Se o preÃ§o atual estÃ¡ dentro da janela crÃ­tica:
        // [HorÃ¡rio_NotÃ­cia - m_minutes_before] atÃ© [HorÃ¡rio_NotÃ­cia + m_minutes_after]
        if(seconds_to >= -block_after_sec && seconds_to <= block_before_sec) {
            is_blocked = true;
            out_action = (int)m_action;

            // Se for o evento ativo ou mais prÃ³ximo no futuro, salva os dados
        ...
        // Se ainda nÃ£o bloqueou mas Ã© o mais prÃ³ximo no futuro
            ...
            m_next_news.importance = (int)event.importance;
            m_next_news.seconds_to = seconds_to;
            m_next_news.is_active  = false; // Ainda nÃ£o bloqueado
        }
    }
