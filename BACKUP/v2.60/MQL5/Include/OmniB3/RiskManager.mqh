...
//|                Omni-B3 EA v2.60 — Gestão de Risco (B3/BRL)        |
...
private:
    ...
    //+--------------------------------------------------------------+
        ...
        if(now.day != m_last_day) {
            ...
            // Se estiver rodando no Testador de Estrategias (Backtest),
            // reseta o Kill-Switch automaticamente na virada do dia para continuidade do teste!
            if(MQLInfoInteger(MQL_TESTER)) {
                if(m_kill_switch) {
                    m_kill_switch = false;
                    m_logger.Info("RiskManager", "[TESTER] Resetando Kill-Switch automaticamente na virada do dia para continuidade do backtest.");
                }
            }
            
            m_logger.Info("RiskManager",
                StringFormat("[RESET] Novo dia - Saldo=R$%.2f | Equity=R$%.2f",
...
public:
    ...
    //+--------------------------------------------------------------+
        ...
        // 1. Kill-Switch (trava absoluta de emergência) - SEMPRE BLOQUEIA

        if(m_kill_switch) {

            static datetime last_log = 0;

            if(TimeCurrent() - last_log >= 600) {

                m_logger.Warning("RiskManager", "[BLOQUEIO] Operaço bloqueada pelo Kill-Switch ativo (EA desativado).");
        ...
        // 2. Bloqueio diário - Bloqueia apenas NOVAS séries
        ...
        if(m_daily_locked) {
            ...
            if(current_levels == 0) {
                ...
                if(TimeCurrent() - last_log >= 600) {
                    ...
                    m_logger.Warning("RiskManager", "[BLOQUEIO] Operaço bloqueada devido a algum limite diário de proteço atingido hoje.");
        ...
        // 3. Bloqueio por limite atual (ciclo ativo) - Bloqueia apenas NOVAS séries
        ...
        // 4. Equity Stop (Bloqueio crítico de liquidez) - SEMPRE BLOQUEIA

        if(balance > 0 && (equity / balance * 100.0) < m_equity_stop_percent) {

            m_logger.Critical("RiskManager",

                StringFormat("[ALERTA] EQUITY STOP! Capital Líquido (R$%.2f) caiu abaixo do limite de %.1f%% do Saldo (R$%.2f) - Atual: %.1f%%. Ativando Kill-Switch!",
        ...
        if(m_initial_balance > 0) {
            ...
            // Re-calcula drawdown diário máximo registrado
            ...
            if(m_max_daily_dd_percent > 0 && daily_dd > m_max_daily_dd_percent) {
                ...
                if(!m_daily_locked) {
                    ...
                    m_logger.Warning("RiskManager",
                        ...
                        StringFormat("[ALERTA] DD Diário Atingido: %.2f%% de drawdown (Limite Máximo: %.2f%%). Trava diária ativada para preservar capital inicial de R$%.2f.",
                ...
                if(current_levels == 0) return false;
            ...
            if(m_limit_loss_daily > 0.0 && daily_loss >= m_limit_loss_daily) {
                ...
                if(!m_daily_locked) {
                    ...
                    m_logger.Warning("RiskManager",
                        ...
                        StringFormat("[ALERTA] Perda diária limite atingida: R$%.2f de perda flutuante/realizada (Limite Máximo: R$%.2f). Trava diária ativada.",
            ...
            if(m_limit_profit_daily > 0.0 && (equity - m_initial_balance) >= m_limit_profit_daily) {
                ...
                if(!m_daily_locked) {
                    ...
                    m_logger.Info("RiskManager",
                        ...
                        StringFormat("[OK] Meta diária de lucro atingida! Lucro acumulado hoje: R$%.2f (Meta: R$%.2f). Trava diária ativada.",
        ...
        if(m_limit_orders_daily > 0 && m_daily_order_count >= m_limit_orders_daily) {
            ...
            if(!m_daily_locked) {
                ...
                m_logger.Warning("RiskManager",
                    ...
                    StringFormat("[ALERTA] Limite diário de ordens atingido: %d ordens executadas hoje (Limite Máximo: %d). Trava diária ativada.",
        ...
        if(m_limit_wins_daily > 0 && m_daily_wins >= m_limit_wins_daily) {
            ...
            if(!m_daily_locked) {
                ...
                m_logger.Warning("RiskManager",
                    ...
                    StringFormat("[ALERTA] Limite diário de vitórias (trades vencedores) atingido: %d vitórias (Limite Máximo: %d). Trava diária ativada.",
        ...
        if(m_limit_losses_daily > 0 && m_daily_losses >= m_limit_losses_daily) {
            ...
            if(!m_daily_locked) {
                ...
                m_logger.Warning("RiskManager",
                    ...
                    StringFormat("[ALERTA] Limite diário de perdas (trades perdedores) atingido: %d perdas (Limite Máximo: %d). Trava diária ativada.",
        ...
        // 7. Máximo de posições/níveis simultâneos da grade - SEMPRE BLOQUEIA
        ...
        // 8. Margem livre da corretora (Margem Livre / Margem Exigida) - SEMPRE BLOQUEIA
            ...
            if(margin_level < m_min_margin_percent) {
                ...
                if(TimeCurrent() - last_log >= 600) {
                    ...
                    m_logger.Warning("RiskManager",
                        ...
                        StringFormat("[ALERTA] Margem Livre Insuficiente na Corretora! Margem livre calculada em %.1f%% do capital garantido, inferior ao limite de segurança configurado de %.1f%%. Novas entradas bloqueadas para evitar Stop Out compulsório pela corretora.", 
        ...
        // 9. Limites de conta (saldo e equity mínimos) - Bloqueia apenas NOVAS séries
        ...
        // 10. Meta de saldo/equity máximos (para quando atingir) - Bloqueia apenas NOVAS séries
